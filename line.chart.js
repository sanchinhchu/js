    ...
    Plugin.prototype = {
        ...
        drawAxis: function (c, graph) {
            ...
            if (config.xaxislabel) {
                c.fillText(config.xaxislabel, graph.width() / 2, graph.height() - 10);
            }
        ...
        drawBar: function (c, graph) {
            ...
            for (var i = 0; i < config.data[0].values.length; i++) {
                ...
                c.beginPath();
                c.rect(that.pixelX(i, 0) - config.yPadding / 4, that.pixelY(config.data[0].values[i].Y) + 30, config.yPadding / 2, graph.height() - that.pixelY(config.data[0].values[i].Y) - config.xPadding - 20);
                c.closePath();
                c.stroke();
                c.fill();
                c.textAlign = "left";
                c.fillStyle = "#000";
                c.fillText(config.data[0].values[i].Y, that.pixelX(i, 0) - config.yPadding / 4, that.pixelY(config.data[0].values[i].Y) + 20, 200);
            }
        },
